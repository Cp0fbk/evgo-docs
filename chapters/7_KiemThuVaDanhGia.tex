...
\begin{itemize}
    \item \textbf{Kiểm thử đơn vị:} Xác thực tính đúng đắn của từng đơn vị logic nghiệp vụ tại tầng dịch vụ (Service) và lắng nghe sự kiện (Event Listener) trong điều kiện cô lập hoàn toàn với cơ sở hạ tầng bên ngoài.

    \item \textbf{Kiểm thử thủ công:} Kiểm tra luồng tích hợp đầu cuối qua giao diện thực tế đã được triển khai, kết hợp với website giả lập phần cứng OCPP ChargePoint Simulator để mô phỏng tương tác với trụ sạc vật lý.

    \item \textbf{Kiểm thử hiệu năng:} Đánh giá khả năng chịu tải thực tế của hệ thống dưới điều kiện truy cập đồng thời quy mô lớn thông qua Apache JMeter~\cite{jmeter}.
\end{itemize}
...
\hspace*{\parindent}
Trọng tâm của giai đoạn kiểm thử đơn vị là xây dựng một bộ kịch bản kiểm thử
...
% ------------------------------------------------------------
\subsubsection{Kết quả kiểm thử đơn vị}
...
\begin{figure}[H]
    ...
    \caption{Kết quả chạy toàn bộ bộ kiểm thử đơn vị qua Maven
...
\begin{center}
    ...
    \begin{longtable}{|>{\raggedright\arraybackslash}m{0.15\textwidth}|>{\raggedright\arraybackslash}m{0.5\textwidth}|c|c|}
        \caption{Tổng hợp kịch bản kiểm thử đơn vị theo module}
...
% ------------------------------------------------------------
\subsection{Kiểm thử thủ công (Manual Testing)}
% ------------------------------------------------------------
